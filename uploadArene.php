...
if (isset($_FILES['image'])) {
	if (!$_FILES['image']['error']) {
		...
		if ($poids < 1000000) {
			...
			if ($poids < MAX_FILE_SIZE) {
				...
				if (isset($extensions[$fileType])) {
					...
					mysql_query('INSERT INTO `arenes` SET identifiant='.$identifiants[0].',identifiant2='.$identifiants[1].',identifiant3='.$identifiants[2].',identifiant4='.$identifiants[3]);
					$id = mysql_insert_id();
					move_given_file($_FILES['image']['tmp_name'], 'images/uploads/course'.$id.'.'.$ext);
					header('Location: course.php?i='.$id.'&uploaded=1');
					exit;
				}
		...
		if ($isUploaded)
			$error = $language ? 'An error occured during the image transfer. Please try again later.':'Une erreur est survenue lors de l\'envoi de l\'image. Réessayez ultèrieurement.';
		else
			$error = $language ? 'The url address you provided does not exist.':'L\'adresse URL que vous avez fournie n\'existe pas.';
	}
}
if (isset($error))
