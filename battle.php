...
if ($canChange) {
	...
	function saveRace() {
		...
		xhr("<?php echo ($isMCup ? 'saveMCup' : ($isCup?'saveCup':'saveBattle')); ?>.php", "<?php
			...
			if (reponse && !isNaN(reponse)) {
				...
				cP.innerHTML = '<?php
					if ($cShared)
						echo $language ? 'The sharing of your '. ($isCup ? 'cup':'arena') .' has been updated.':'Le partage de votre '. ($isCup ? 'coupe':'arène') .' a été mis à jour.';
					else
						echo $language ? 'Your '. ($isCup ? 'cup':'arena') .' has just been added to the <a href="creations.php" target="_blank">list</a> !':'Votre '. ($isCup ? 'coupe':'arène') .' vient d\\\'être ajouté à la <a href="creations.php" target="_blank">liste</a> !';
				?><br /><br />';
				...
				if ($isCup) {
					...
					document.getElementById("changeRace").onclick = function() {
						document.location.href = "<?php echo $isMCup ? 'completecups.php?mid=':'completecup.php?cid='; ?>"+ reponse +"<?php echo $isCup ? '&battle':''; ?>";
					};
					<?php
				}
				?>
	...
	function supprRace() {
		...
		xhr("<?php echo ($isMCup ? 'supprMCup':($isCup ? 'supprCup':'supprBattle')); ?>.php", "id=<?php echo $nid; ?>", function(reponse) {
			if (reponse == 1) {
...
elseif ($canChange) {
	...
	?>
	<input type="button" id="changeRace" onclick="document.location.href='<?php echo ($isCup ? ($isMCup ? 'completecups.php':'completecup.php'):'course.php') ?>'+document.location.search<?php if ($isCup) echo '+\'&battle\''; ?>" value="<?php echo ($language ? 'Edit '.$typeStr:'Modifier '.$typeStr); ?>" /><br /><br /><?php
	if (!$cShared) {
		?>
	&nbsp;
		<?php
