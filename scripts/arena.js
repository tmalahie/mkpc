...
function startMapMusic(lastlap) {
	if (lastlap)
		...
		mapMusic = startMusic("musics/maps/map9.mp3");
...
resetScreen();
	...
	for (var i=0;i<strPlayer.length;i++) {
		var oPlayer = {
			x : oMap.startposition[i][0]%6*100+50,
			y : Math.floor(oMap.startposition[i][0]/6)*100+50,
			z : 0,
			...
			rotation : oMap.startposition[i][1]*90,
			rotincdir : 0,
			rotinc : 0,
			changeView : 0,
	...
	for (var i=0;i<aPlayers.length;i++) {
		...
		if (joueur != strPlayer[0] && joueur != strPlayer[1]) {
			var oEnemy = {
				...
				rotation : oMap.startposition[iAI][1]*90,
				rotincdir : 0,
				rotinc : 0,

				x : oMap.startposition[iAI][0]%6*100+50,
				y : Math.floor(oMap.startposition[iAI][0]/6)*100+50,
				z : 0,
				...
				cpu : true,
				aipoint : oMap.startposition[iAI][0],
				lastAI : oMap.startposition[iAI][0] + corDir[oMap.startposition[i][1]],
				maxspeed : 5.7,
...
function BGLayer(strImage, scaleFactor) {
	...
	function render() {
		...
		if (!i) {
			...
			if (fSprite[4] && (touche_banane(roundX1, roundY1) || touche_banane(roundX2, roundY2) || touche_crouge(roundX1, roundY1) || touche_crouge(roundX2, roundY2) || touche_cverte(roundX1, roundY1, j) || touche_cverte(roundX2, roundY2, j)))
		...
		if (fSprite[1] > oBox[0] - 10 && fSprite[1] < oBox[0] + 10 && fSprite[2] > oBox[1] - 10 && fSprite[2] < oBox[1] + 10)
			...
			if (i || ((fSprite[6] == undefined || canMoveTo(fSprite[1],fSprite[2], fMoveX,fMoveY)) && !touche_banane(fNewPosX, fNewPosY) && !touche_banane(fSprite[1], fSprite[2]) && !touche_crouge(fNewPosX, fNewPosY, j) && !touche_crouge(fSprite[1], fSprite[2], j) && !touche_cverte(fNewPosX, fNewPosY) && !touche_cverte(fSprite[1], fSprite[2]))) {
				fSprite[1] = fNewPosX;
				fSprite[2] = fNewPosY;
...
function supprArme(i)	{
	...
	for (var i=0;i<aKarts.length;i++)	{
		var kart = aKarts[i];
		if (kart != oKart && Math.pow(kart.x-oKart.x, 2) + Math.pow(kart.y-oKart.y, 2) < 25 && !kart.tourne && (!kart.protect || (!kart.etoile && oKart.etoile))) {
			loseBall(i);
			stopDrifting(i);
			spinKart(kart,62);
			if (kart.using[0]) {
...
if (!oKart.cpu)	{
	...
	if (oKart.tombe)	{
		...
		if (!oKart.z) {
			...
			else if ((oKart.x > 599) || (oKart.y > 599) || (oKart.x < 0) || (oKart.y < 0))	{
			if (oKart.x > 599) {
				oKart.x = 550;
				if (oKart.y > 599) {
					oKart.y = 550;
					...
					oKart.y = oKart.y-oKart.y%100+50;
					oKart.rotation = 270;
				}
			}
			else if (oKart.y > 599) {
				...
				if (oKart.x < 0) {
					...
					oKart.x = oKart.x-oKart.x%100+50;
					oKart.rotation = 180;
				}
			}
			else if (oKart.x < 0) {
	...
	function ai(oKart) {
		var aCurPoint = [(oKart.aipoint%6)*100+50,Math.floor(oKart.aipoint/6)*100+50];
		...
		if (nbPos != oKart.lastAI) {
			var chemins = oMap.aipoints[nbPos];
...
if (!pause)	{
	...
	function selectNbJoueurs() {
		...
		for (i=1;i<=2;i++) {
			...
			oPInput.style.left = ((nid?26:27)*iScreenScale)+"px";
			oPInput.style.top = (((nid?7:10)+i*(nid?7:8))*iScreenScale)+"px";
			if (nid) {
		...
		if (nid) {
			...
			oPInput.onclick = function() {
				document.location.href = "online.php?id="+ nid +"&battle";
			}
			oScr.appendChild(oPInput);
		}
