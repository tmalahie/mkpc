...
function o_launchchat(memberID,memberPseudo) {
	...
	o_xhr("startchat.php", "member="+ memberID, function(reponse) {
		...
		o_startchat(memberID,memberPseudo,reponse,true);
		return true;
	});
}
...
function o_startchat(memberID,pseudo,messages,autofocus) {
	var o_chat = document.getElementById("chatwindow"+ memberID);
	if (o_chat) {
...
function o_repond(dem,name,rep,elt,battle) {
	o_prompt(rep ? (o_language ? "Send an additionnal message to "+ name +" (optionnal)":"Envoyer un message en compl\xE9ment (facultatif)") : (o_language ? "Send a message to explain your refusal (optional)":"Envoyer un message pour expliquer votre refus (facultatif)"),
