...
function loadMap() {
	...
	oMapImg.src = "mapcreate.php"+ oMap.map;
	...
	for (var i=0;i<strPlayer.length;i++) {
		...
		document.getElementById("compteur"+i).innerHTML = toLanguage('Lap', 'Tour') +' <span id="tour'+i+'">1</span>/'+ oMap.tours;
...
function arme(ID) {
	...
	if (!oKart.using[0]) {
		...
		case "carapace" :
			...
			playIfShould(oKart,"musics/events/throw.mp3");
			break;
...
function startMapMusic(lastlap) {
	if (lastlap)
		...
		mapMusic = startMusic("musics/maps/map"+ oMap.type +".mp3");
...
function startGame() {
	...
	for (var i=0;i<strPlayer.length;i++) {
		...
		case -6 :
			oPlayer.x += ((aPlayers.length-i)%2 ? 0 : 18);
			oPlayer.y += (aPlayers.length-i) * 12;
			break;
		case -1 :
			oPlayer.x += (aPlayers.length-i) * 12;
			oPlayer.y -= ((aPlayers.length-i)%2 ? 0 : 18);
			break;
		case 6 :
			oPlayer.x -= ((aPlayers.length-i)%2 ? 0 : 18);
			oPlayer.y -= (aPlayers.length-i) * 12;
	...
	if (course != "CM") {
		...
		for (var i=0;i<aPlayers.length;i++) {
			...
			if (joueur != strPlayer[0] && joueur != strPlayer[1]) {
				...
				switch (oMap.startdirection) {
				case -6 :
					oEnemy.x += ((aPlayers.length-iAI)%2 ? 0 : 18);
					oEnemy.y += (aPlayers.length-iAI) * 12;
					break;
				case -1 :
					oEnemy.x += (aPlayers.length-iAI) * 12;
					oEnemy.y -= ((aPlayers.length-iAI)%2 ? 0 : 18);
					break;
				case 6 :
					oEnemy.x -= ((aPlayers.length-iAI)%2 ? 0 : 18);
					oEnemy.y -= (aPlayers.length-iAI) * 12;
			...
			for (var j=0;j<aKarts.length;j++) {
				...
				if (oKart.personnage == aPlayers[i]) {
					...
					case -6 :
						...
						break;
					case -1 :
						oKart.x = oMap.startposition[0] + (i+1) * 12;
						oKart.y = oMap.startposition[1] - (i%2 ? 18 : 0);
						break;
					case 6 :
						oKart.x = oMap.startposition[0] - (i%2 ? 18 : 0);
						oKart.y = oMap.startposition[1] - (i+1) * 12;
	...
	if (strPlayer.length == 1) {
		...
		oPlanDiv2.style.top = (10 + Math.round(iScreenScale/4) + oPlanWidth2) +"px";
		oPlanDiv2.style.width = oPlanWidth2 +"px";
		oPlanDiv2.style.height = oPlanWidth2 +"px";
...
function continuer() {
	...
	if (course != "CM") {
		...
		oSave.onclick = function() {
			...
			oForm.onsubmit = function() {
				...
				if (nom) {
					...
					xhr("records.php", "nom="+nom+"&perso="+strPlayer[0]+"&creation="+ oMap.id +"&temps="+iTrajet.length, function(reponse) {
						if (reponse) {
							document.body.style.cursor = "default";
							var enregistre;
							try {
		...
		oClassement.onclick = function() {
			open("classement.php?circuit="+ oMap.id);
		}
...
function tombe(iX, iY, iC) {
	if (iX > 600 || iY > 600 || iX < 0 || iY < 0) return [oMap.startposition[0],oMap.startposition[1],oMap.startposition[2]];
...
function places(j) {
	...
	for (var i=0;i<aKarts.length;i++) {
		...
		if (dest == oMap.checkpoint.length)
			dest = 0;
...
function checkpoint(kart, aPosX,aPosY) {
	...
	for (var i=0;i<oMap.checkpoint.length;i++) {
		...
		if (kart.x > oBox[0] && kart.x < oBox[0] + (oBox[3] ? oBox[2] : 15) && kart.y > oBox[1] && kart.y < oBox[1] + (oBox[3] ? 15 : oBox[2])) {
			if (aPosX > oBox[0] && aPosX < oBox[0] + (oBox[3] ? oBox[2] : 15) && aPosY > oBox[1] && aPosY < oBox[1] + (oBox[3] ? 15 : oBox[2]))
				continue;
...
function move(getId) {
	...
	if (!oKart.z) {
		...
		if (pJump && !oKart.tourne) {
			...
			var hpType;
			var fTombe = tombe(Math.round(oKart.x), Math.round(oKart.y), oMap.checkpoint[(oKart.demitours+1!=oMap.checkpoint.length) ? oKart.demitours+1 : 0][3]);
			if (fTombe) {
				oKart.x = fTombe[0];
				oKart.y = fTombe[1];
					...
					for (var mAI=1;tAI;mAI++) {
						...
						for (var i=0;i<50;i++) {
							...
							if (!oKart.tourne && oKart.speed && !oKart.billball) {
								...
								if (oKart.rotation > 360)
									oKart.rotation -= 360;
								oKart.x += oKart.speed * direction(0, oKart.rotation);
								oKart.y += oKart.speed * direction(1, oKart.rotation);
							}
							if (!oKart.z && !oKart.heightinc) {
								if (oKart.speed < 5)
									oKart.speed += oKart.speedinc;
						...
						if (!tAI)
							oKart.aipoint++;
						if (oKart.aipoint < 0) {
							oKart.aipoint += oMap.aipoints.length;
							if (oKart.aipoint < 0) {
								oKart.aipoint = fAI;
								tAI = false;
							}
	...
	if (oMap.aipoints.length && checkpoint(oKart,aPosX,aPosY)) {
		var nbjoueurs = aKarts.length;
					...
					if (course != "CM") {
						...
						oContinue.onclick = function() {
							...
							function askForRegister() {
								aPara1.innerHTML = toLanguage('Save the time to the <a href="classement.php?circuit='+ oMap.id +'" target="_blank" style="color: orange">record list</a> ?', 'Enregistrer le temps dans la <a href="classement.php?circuit='+ oMap.id +'" target="_blank" style="color: orange">liste des records</a> ?');
								oSave.onclick = function() {
									document.body.removeChild(oForm);
									continuer();
	...
	if (oKart.cpu) {
		if (oKart.roulette == 25 || oKart.using[0] && !oKart.tourne) {
			...
			if (oKart.arme != "champi" && oKart.arme != "megachampi" && oKart.arme != "etoile") {
				if (Math.random() > 0.98)
					arme(getId);
			}
			else {
		...
		if (oKart.maxspeed > iDificulty*1.25)
			oKart.maxspeed = iDificulty*1.25;
		else if (oKart.maxspeed < iDificulty)
			oKart.maxspeed = iDificulty;
...
function selectTypeScreen() {
	...
	for (i=0;i<oModes.length;i++) {
		...
		oPInput.onclick = function() {
			...
			if (course == "CL")
				document.location.href = "online.php?"+(isCup?"sid":"id")+"="+nid;
			else {
				FBRoot.style.display = "none";
				oScr.innerHTML = "";
...
function selectPlayerScreen(IdJ,newP) {
	...
	if (course == "VS") {
		...
		oClassement.onclick = function() {
			open("classement.php"+ (isCup ? "?scup="+nid : "?circuit="+nid));
		};
...
function setMapSrc(oPImg,i) {
	setTimeout(function() {
		oPImg.src = "trackicon.php?id="+ oMaps[i].id +"&type=0";
	}, 100*i);
}
