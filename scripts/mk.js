...
function updatePlanFullScreen() {
	...
	if (isFS && oPlanDiv2.parentNode !== oContainers[0]) {
		...
		oPlanDiv2.style.top = Math.round(iScreenScale * ($speedometers.length ? 4.8 : 2.5)) +"px";
			...
			oForm.onsubmit = function() {
				...
				if (nom) {
					...
					if (savedRecord)
						...
						xhr("trackStats.php", "stats="+encodeURIComponent(getRecordStats(params)), function(res) {
...
function move(getId, triggered) {
	...
	var fMaxKartSpeed = oKart.maxspeed * sizeSpeedRatio(oKart) * fSelectedClass;
	...
	if (oKart.cpu) {
		if (course == "BB")
			...
			if (oKart.place <= oPlayerPlace)
				oKart.maxspeed0 -= (oKart.maxspeed0*influence-rSpeed*sizeSpeedRatio(oKart)*fSelectedClass)/100;
			else
				oKart.maxspeed0 += (rSpeed*rRatio*1.12*sizeSpeedRatio(oKart)*fSelectedClass-oKart.maxspeed0*influence)/100;
		}
	...
	if (oKart.cannon) {
		...
		if (oKart.billball)
			oKart.speed = 20;
		oKart.maxspeed = oKart.speed/(sizeSpeedRatio(oKart)*fSelectedClass);
		if (!oKart.speedinc)
			oKart.speedinc = 0.01;
...
function processCode(cheatCode) {
	...
	if (isLap) {
		...
		if (isNaN(t) || isNaN(c))
			return false;
		var prevLap = oPlayer.tours;
		oPlayer.tours = t;
		oPlayer.demitours = c;
		updateLapHud(0);
		handleLapChange(prevLap, 0);
		return true;
	}
	if (cheatCode == "pos") {
...
function cappedRelSpeed(oKart) {
	var fSize = (oKart && oKart.size!==undefined) ? sizeSpeedRatio(oKart) : 1;
