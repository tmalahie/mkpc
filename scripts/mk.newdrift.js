...
function loadMap() {
	var mapSrc = isCup ? (complete ? "images/uploads/"+ (course=="BB"?"course":"map") + oMap.map +"."+ oMap.ext:"mapcreate.php"+ oMap.map):"images/maps/map"+ oMap.map +".png";
	if (mapSrc.match(/\.gif$/g)) {
		oMapImg = GIF();
		oMapImg.onloadall = startGame;
		oMapImg.load(mapSrc);
	}
	else {
		oMapImg = new Image();
...
function startGame() {
	...
	if (isOnline) {
		...
		setTimeout(fncCount,bMusic?3000*0.01:1500*0.01);
	if (oMapImg.image) {
...
function Sprite(strSprite) {
	...
	for (var i=0;i<strPlayer.length;i++) {
		...
		this[i].draw = function(iX, iY, fScale, iZ) {
			...
			if (oY > iHeight * iScreenScale || (iY+iZ*iScreenScale) < 9 * iScreenScale) {
...
var decorBehaviors = {
	...
	firesnake:{
		...
		move:function(decorData,i) {
			...
			case 1:
				...
				decorData[0] = decorData[9][0] + l*(decorData[8][0]-decorData[9][0]);
				decorData[1] = decorData[9][1] + l*(decorData[8][1]-decorData[9][1]);
				break;
			case 2:
				decorData[4]--;
				if (decorData[4] <= 0) {
	...
	fireplant:{
		...
		init:function(decorData,i) {
			for (var j=0;j<strPlayer.length;j++) {
				decorData[2][j].nbSprites = 2;
				decorData[2][j].w = 80;
				decorData[2][j].h = decorData[2][j].w*67/63;
				decorData[2][j].z = 0.3;
			}
			if (decorData[4] == undefined)
		...
		move:function(decorData,i) {
			...
			else if (decorData[5] == -3) {
				for (var j=0;j<strPlayer.length;j++)
					decorData[2][j].setState(0);
				decorData[5] = Math.round(70 + 10*Math.sin(decorData[7]));
			}
		}
	},
	fireball:{
	...
	billball:{
		...
		move:function(decorData) {
			...
			for (var j=0;j<oPlayers.length;j++) {
				var fAngle = nearestAngle(getApparentRotation(oPlayers[j]), 180,360);
	...
	mariotree:{
		...
		init:function(decorData) {
			for (var j=0;j<strPlayer.length;j++) {
				decorData[2][j].nbSprites = 1;
				decorData[2][j].h = 104;
				decorData[2][j].w = decorData[2][j].h*39/52;
				decorData[2][j].z = 0.25;
			}
		}
	},
	fir:{
		...
		init:function(decorData) {
			for (var j=0;j<strPlayer.length;j++) {
				decorData[2][j].nbSprites = 1;
				decorData[2][j].h = 112;
				decorData[2][j].w = decorData[2][j].h*45/86;
				decorData[2][j].z = 0.14;
			}
		}
	},
	movingtree:{
		...
		move:function(decorData) {
			if (decorData[6])
				...
				if (diffX*diffX+diffY*diffY < 1) {
					...
					var diffL = Math.hypot(diffX,diffY);
					decorData[0] += diffX*0.7/diffL;
					decorData[1] += diffY*0.7/diffL;
				}
			}
		}
	},
	...
	pokey:{
		...
		init:function(decorData,i) {
			...
			if (decorData[5] == undefined)
				decorData[5] = [(10000*Math.sin(i+1))%Math.PI,0.025];
			if (decorData[6] == undefined)
	...
	falltree:{
		...
		init:function(decorData) {
			for (var j=0;j<strPlayer.length;j++) {
				decorData[2][j].nbSprites = 1;
				decorData[2][j].w = 100;
				decorData[2][j].h = decorData[2][j].w*62/54;
				decorData[2][j].z = 0.32;
			}
		}
	},
	peachtree:{
		...
		init:function(decorData) {
			for (var j=0;j<strPlayer.length;j++) {
				decorData[2][j].nbSprites = 1;
				decorData[2][j].w = 27;
				decorData[2][j].h = decorData[2][j].w*67/17;
				decorData[2][j].z = 0.03;
			}
		}
	},
	box:{
	...
	snowball:{
		...
		init:function(decorData) {
			for (var j=0;j<strPlayer.length;j++) {
				decorData[2][j].nbSprites = 1;
				decorData[2][j].w = 56;
				decorData[2][j].h = decorData[2][j].w*53/49;
				decorData[2][j].z = 0.32;
				decorData[3] = 0;
				decorData[4].unshift([decorData[0],decorData[1]]);
				decorData[5] = [1,0];
	...
	truck:{
		...
		movable:true,
		rotatable:true,
		path:[[[719,526],[904,523],[956,511],[1000,486],[1028,451],[1039,397],[1029,351],[996,318],[946,301],[887,309],[844,334],[803,393],[780,416],[747,427],[653,428],[604,401],[593,366],[611,329],[715,270],[745,236],[755,200],[742,162],[708,138],[660,141],[286,380],[235,425],[222,460],[224,489],[239,513],[281,563],[290,598],[284,632],[176,721],[162,751],[159,855],[167,910],[180,947],[204,979],[238,995],[289,997],[318,982],[332,960],[337,937],[325,912],[275,864],[256,820],[266,783],[294,756],[343,735],[427,735],[453,716],[468,694],[472,603],[483,559],[510,534],[551,527]],[[714,542],[536,544],[504,558],[494,579],[485,691],[468,726],[437,750],[418,754],[351,754],[296,781],[282,808],[287,835],[299,860],[331,895],[350,920],[353,943],[347,974],[326,997],[300,1011],[258,1016],[202,997],[180,980],[156,938],[140,854],[149,730],[173,693],[244,645],[265,610],[262,570],[208,511],[194,473],[205,426],[236,389],[661,117],[698,113],[733,123],[762,156],[771,192],[764,237],[732,282],[631,342],[622,365],[627,389],[653,408],[698,413],[757,409],[786,390],[818,336],[851,304],[901,281],[958,278],[1022,300],[1057,350],[1065,397],[1051,462],[1015,502],[959,530],[905,540]]],
		init:function(decorData) {
			for (var j=0;j<strPlayer.length;j++) {
	...
	chomp:{
		...
		init: function(decorData,i) {
			for (var j=0;j<strPlayer.length;j++) {
				decorData[2][j].nbSprites = 1;
				decorData[2][j].w = 72;
				decorData[2][j].h = decorData[2][j].w*47/44;
				decorData[2][j].z = 0.32;
			}
			if (decorData[3] == undefined)
	...
	for (var i=0;i<strPlayer.length;i++) {
		if (oPlayers[i].tombe <= 10) {
			...
			for (var j=0;j<bananes.length;j++){
				...
				fSprite[0][i].draw(
					((iWidth/2) + fViewX) * iScreenScale, 
					(iHeight - iViewY - correctCamZ(fSprite[5], iDeltaX)) * iScreenScale,
...
function colKart(getId) {
	...
	for (var i=0;i<getId;i++) {
		var kart = aKarts[i];
		if (!friendlyFire(oKart,kart) && (course!="BB"||(oKart.ballons.length&&kart.ballons.length)) && Math.pow(oKart.x-kart.x, 2) + Math.pow(oKart.y-kart.y, 2) < 200 && (oKart.z<=1.175||oKart.billball) && (kart.z<=1.175||kart.billball) && !oKart.tourne && !kart.tourne) {
			...
			var hitboxSize = 2.5*(kart.size+oKart.size)*(kart.size+oKart.size);
					...
					if (d20 > hitboxSize) {
						if (!oKart.cpu || !kart.cpu) {
							...
							if (del) {
								...
								nDir2[0] *= l2;
								nDir2[1] *= l2;
								var sizeRatio = kart.size/oKart.size;
								var pushVector = [nDir1[0]*sizeRatio,nDir1[1]*sizeRatio];
								...
								pushVector = [(nDir2[0]+dir1[0]-dir2[0])/sizeRatio,(nDir2[1]+dir1[1]-dir2[1])/sizeRatio];
				...
				if (nX > oBox[0]-hitboxSize && nX < oBox[0]+hitboxSize && nY > oBox[1]-hitboxSize && nY < oBox[1]+hitboxSize && (Math.abs((oBox[3]?oBox[3]:0)-iZ)<hitboxHeight)) {
					if (!iP || decorBehavior.unbreaking) {
						if (collisionTest == COL_KART) {
							if (decorBehavior.spin && !iP)
								collisionSpin = decorBehavior.spin;
...
var collisionTest, collisionPlayer, collisionTeam, collisionSpin;
...
function move(getId) {
	...
	if (!oKart.tourne && oKart.speed && !oKart.billball && !oKart.figstate && !oKart.cannon)
		...
		oKart.figstate = 0;
		oKart.speed = oKart.speed/1.5-0.5;
		oKart.tourne -= 2;
	...
	if ((!getId || !isOnline || finishing) && !oKart.loose) {
		...
		if (pExplose && !oKart.tourne && !oKart.protect) {
			...
			else if (!oKart.protect && !oKart.tourne) {
	...
	if (collisionSpin && !oKart.tourne) {
		if (Math.abs(oKart.speed) > 2.5) {
			loseBall(getId);
			stopDrifting(getId);
	...
	if (!oKart.z) {
		...
		if (pJump && !oKart.tourne) {
			...
			if (fTombe) {
				...
				if (!oKart.protect && !oKart.champi && !oKart.figuring && oKart.speed > 1.5 && !(oKart.turbodrift>oKart.turbodrift0*0.8) && (hpType=ralenti(posx_arrondi, posy_arrondi))) {
					var capSpeed;
					switch (hpType) {
						case "herbe" :
				...
				if (oiling(posx_arrondi, posy_arrondi) && (Math.abs(oKart.speed)>0.5) && !oKart.tourne && !oKart.protect && !oKart.z) {
					stopDrifting(getId);
					oKart.spin(20);
				}
				newShift = flowShift(posx_arrondi, posy_arrondi, oKart.protect);
			}
			oKart.figuring = false;
			oKart.figstate = 0;
		}
	...
	if (oKart.cpu) {
		if (course == "BB")
			...
			var manyPlayers = ((iDificulty >= 5) && (aKarts.length > 8));
			if (!manyPlayers)
...
function ai(oKart) {
	...
	if ((oMap.sections && oKart.tours > oMap.tours) || (oKart.ballons && !oKart.ballons.length) || oKart.tourne) {
		oKart.speedinc = 0;
		oKart.rotinc = 0;
		return;
	}
	...
	for (var f=0;f<oKart.aipoints.length;f++) {
		...
		if (oKart.collided) {
			...
			nTheta = nearestAngle(nTheta*180/Math.PI, oKart.rotation, 360);
			if (isNaN(nTheta)) nTheta = oKart.rotation;
			var diffTheta = nTheta-oKart.rotation;
			...
			if (diffThetaAbs > fMaxRotInCp) {
				...
				if (!maxSpeed)
					maxSpeed = 0.01;
				oKart.speedinc = Math.max(Math.min(maxSpeed-oKart.speed,1),-1);
				if (oKart.speedinc < 0)
...
function selectPlayerScreen(IdJ,newP,nbSels) {
	...
	function tourner(kart) {
		...
		var rotation = size*Math.round(parseFloat(kart.style.left)/size);
		if (rotation > -21*size)
		...
		if (pUnlocked[i]) {
			...
			oPImg.onmouseout = function() {
				...
				if (this.naturalWidth) {
					...
					this.style.left = -(30 * jScreenScale) +"px";
				clearTimeout(rotateHandler);
			}
