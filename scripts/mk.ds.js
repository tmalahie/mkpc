...
function loadMap() {
	var mapSrc = isCup ? (complete ? "images/uploads/"+ (course=="BB"?"course":"map") + oMap.map +"."+ oMap.ext:"mapcreate.php"+ oMap.map):"images/newtracks/map"+oMap.map+"."+oMap.ext;
...
function getMapSelectorSrc(i) {
	...
	isds ? ("images/newselectors/select_" + aAvailableMaps[i] + ".png?reload=1"):("images/newselectors/select_map" + oMaps[aAvailableMaps[i]].map + ".png");
}
