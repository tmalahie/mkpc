...
function setPlanPos() {
	...
	for (var i=0;i<carapaces.length;i++) {
		setObject(oPlanCarapaces[i],carapaces[i][3],carapaces[i][4], oObjWidth,oPlanSize, carapaces[i][2],200);
		setObject(oPlanCarapaces2[i],carapaces[i][3],carapaces[i][4], oObjWidth2,oPlanSize2, carapaces[i][2],200).style.zIndex = 2;
	}
...
function arme(ID) {
	...
	if (!oKart.using[0]) {
		...
		switch(oKart.using[2]) {
			...
			case "carapace" :
			addNewItem(oKart,carapaces,[new Sprite("carapace"), -1, oKart.team, posX + 15 * direction(0, oKart.rotation),posY + 15 * direction(1, oKart.rotation),0,oKart.rotation,10]);
			playDistSound(oKart,"musics/events/throw.mp3",50);
			break;

			case "carapacerouge" :
			addNewItem(oKart,carapacesRouge,[new Sprite("carapace-rouge"), -1, oKart.team, posX + 15 * direction(0, oKart.rotation),posY + 15 * direction(1, oKart.rotation),0,oKart.rotation,oKart.id,-1]);
			playDistSound(oKart,"musics/events/throw.mp3",50);
			break;

			case "bobomb" :
			addNewItem(oKart,bobombs,[new Sprite("bob-omb"), -1, oKart.team, posX,posY,0,oKart.rotation,15,30]);
			playDistSound(oKart,"musics/events/throw.mp3",50);
			break;
...
function startGame() {
	...
	for (var i=0;i<strPlayer.length;i++) {
		...
		if (course != "BB") {
			...
			//oPlayer.demitours = oMap.checkpoint.length-1;
			oPlayer.billball = 0;
	...
	for (i=0;i<aPlayers.length;i++) {
		...
		if (course != "BB") {
			...
			if (simplified)
				oEnemy.lastAI = oMap.startposition[iAIc][3] + corDir[oMap.startposition[iAI][2]];
			...
			if (iCntStep < 3) {
				...
				if (!pause || !fInfos.replay) {
					document.onkeydown = function(e) {
						...
						if (advent__fastforward > timer) return;
						switch (e.keyCode) {
							case 32: // space
								e.preventDefault();
								break;
							...
							case 55:
							case 103:
								if ((13 <= adventCurDay) && !oPlayers[1] && (iDificulty == 5) && !iTeamPlay && (aKarts.length == 8) && !advent__cheated) {
									if ((course == "VS") && !isCup && !isOnline && oMap.map == 1 && !pause) {
										if (!advent__startedat) {
											var tUntil;
											if ((20 <= adventCurDay) && (!isChallengeOpen(13) || (20 == adventCurDay)))
												advent__fastforward = 298;
											else
												advent__fastforward = 223;
										}
										else
					...
					function revoir() {
						...
						setTimeout((timer != iTrajet.length) ? revoir : function(){var oKart=aKarts[0];oKart.aipoint=0;oKart.changeView=180;oKart.maxspeed=5.7;oKart.speed=5.7;oKart.tourne=0;stopDrifting_();stopStunt();document.onkeyup=undefined;document.getElementById("infos0").style.visibility="visible";if(bMusic||iSfx){startEndMusic(0)}cycle()},67);
						render();
					}
					for (i=0;i<aKarts.length;i++)
						aKarts[i].cpu = true;
	...
	if (isOnline) {
		...
		setTimeout(fncCount,bMusic?3000:1500*1.00);
	if (!isOnline)
		document.body.style.cursor = "default";
}
...
function classement() {
	...
	for (var i=0;i<iPlacement.length;i++) {
		...
		document.getElementById("fJ"+i).style.backgroundColor = (iPlayer != 0) ? ((iPlayer != nPlayer) ? (isRedTeam?"red":"") : (isRedTeam?"brown":"navy")) : (isRedTeam?"#F96":"#69F");
	...
	if (course != "CM") {
		if (oMap.ref % 4 || course != "GP") {
			...
			oContinue.onclick = function () {
				...
				if (isChallengeOpen(4) && !advent__cheated) {
					if ((course == "GP") && !isCup && !isOnline && oMap.map == 20) {
						if (aScores[0] == 40)
							challengeSucceeded(4,iScreenScale);
					}
				}
			}
		}
			...
			for (var j=0;j<carapaces.length;j++) {
				...
				if (!i) {
					...
					if (((fSprite[6] != -1) && tombe(roundX1, roundY1)) || touche_banane(roundX1, roundY1) || touche_banane(roundX2, roundY2) || touche_crouge(roundX1, roundY1) || touche_crouge(roundX2, roundY2) || touche_cverte(roundX1, roundY1, j) || touche_cverte(roundX2, roundY2, j)) {
						...
						fSprite[7]--;
						if (fSprite[7]) {
							var fNewTrajectoire = fSprite[6] - (fSprite[6] - 180)*2;
							fMoveX = 8 * direction(0, fNewTrajectoire);
							fMoveY = 8 * direction(1, fNewTrajectoire);
							if (canMoveTo(fSprite[3],fSprite[4], fMoveX,fMoveY))
								fSprite[6] = fNewTrajectoire;
							else {
								fSprite[6] -= (fSprite[6] - 90)*2;
								if (fSprite[6] < 0) fSprite[6] += 360;
							}
						}
						else {
							detruit(carapaces,j);
				...
				if (!fSprite[0][i].div.style.opacity) {
					...
					if (!i && fSprite[6] != -1) {
						...
						if (fSprite[8] != -1) {
							...
							fMoveX = 8 * direction(0, fSprite[6]);
							fMoveY = 8 * direction(1, fSprite[6]);
						}
...
function makeSpriteExplode(fSprite,src,k) {
	...
	fSprite[0][k].img.src = "images/sprites/sprite_"+src+"_smooth.png";
...
function balloonSrc(team) {
	return 'images/sprites/sprite_'+(team==1?'ballonR':'ballon')+'_smooth.png';
}
...
function supprime(cible, id, sound) {
	...
	var mhId = advent__myitems.indexOf(cible[id]);
	if (mhId != -1)
		advent__myitems.splice(mhId,1);
	cible.splice(id,1);
}
...
function handleHit(oBox) {
	if (advent__kart && (advent__kart != oPlayers[0]) && !advent__kart.tourne && (advent__myitems.indexOf(oBox) != -1))
		incAdventHits(advent__kart);
}
function handleHit2(oKart,kart) {
	if ((oKart == oPlayers[0]) && (kart != oPlayers[0]))
		incAdventHits(kart);
}
function incAdventHits(kart) {
	advent__hits++;
	if ((course == "BB") && (kart.ballons.length == 1)) {
		if (advent__killed.indexOf(kart) == -1) {
			advent__killed.push(kart);
			advent__kills++;
		}
	}
	if (advent__hits == 15) {
		if (isChallengeOpen(19) && !oPlayers[1] && !iTeamPlay && (aKarts.length == 8)) {
			if ((course == "BB") && !isCup && !isOnline) {
				challengeSucceeded(19,iScreenScale);
				advent__pauseIfNot();
			}
		}
	}
...
function touche_bobomb(iX, iY, iP) {
	for (var i=0;i<bobombs.length;i++) {
		...
		if (!oBox[5] && i != iP) {
			if (oBox[6] != -1) {
				if (!oBox[7] && iX > oBox[3]-30 && iX < oBox[3]+30 && iY > oBox[4]-30 && iY < oBox[4]+30) {
					if (oBox[8] <= 0) {
						var res = (collisionTeam!=oBox[2]) ? (oBox[8] < -5 ? 42 : 84):false;
						if (res) handleHit(oBox);
						return res;
						//return (collisionTeam!=oBox[2]) ? (oBox[8] < -5 ? 42 : 84):false;
					}
					else
						oBox[8] = 1;
				}
	...
	for (var i=0;i<carapacesBleue.length;i++) {
		...
		if (oBox[6] < 0 && oBox[6] >= -10) {
			if (iX > oBox[3]-30 && iX < oBox[3]+30 && iY > oBox[4]-30 && iY < oBox[4]+30) {
				...
				if (res) handleHit(oBox);
				return res;
				//return (collisionTeam!=oBox[2]) ? (oBox[6] < -5 ? 42 : 84):false;
			}
		}
	}
	return false;
							...
							for (j=0;j<aKarts.length;j++) {
								if (aKarts[j].id == pID) {
									...
									if ((aKarts[j].billball >= 40) && !aBillBall) {
										aKarts[j].sprite[0].img.src = "images/sprites/sprite_billball_smooth.png";
						...
						function displayRankings() {
							...
							for (i=0;i<rCode[3].length;i++) {
								...
								if (pCode[0] == identifiant) {
									oTr.style.backgroundColor = (oPlayers[0].team==1) ? "#F96":"#69F";
							...
							function updateTable() {
								...
								for (i=0;i<oTds.length;i++) {
									...
									oTds[i][1].innerHTML = pCode[2];
									if (pCode[0] == identifiant)
										oTrs[i].style.backgroundColor = (oPlayers[0].team==1) ? "#F96":"#69F";
									else if (pCode[4] == 1)
...
function move(getId) {
	...
	if ((getId<strPlayer.length) && !oKart.cpu && !finishing) {
		showTimer();
		if (timer == 448) {
			if (isChallengeOpen(23) && (oKart == oPlayers[0]) && !advent__cheated) {
				if ((course == "BB") && complete && !isOnline && nid == 479) {
					if (!advent__falls && (oKart.ballons.length+oKart.reserve >= 5)) {
						challengeSucceeded(23,iScreenScale);
						advent__pauseIfNot();
					}
				}
			}
	...
	if (oKart.tombe) {
		...
		if (oKart.tombe == 2) {
			...
			loseBall(getId);
			if (course == "BB" && !oKart.ballons.length && !oKart.loose) {
	...
	if (kartIsPlayer(oKart)) {
		if (!advent__startedat && oKart.speed > 1)
			advent__startedat = timer;
	...
	if (!oKart.z && !oKart.heightinc) {
		...
		if (isCup && oMap.skin != 22) {
			if (oKart.cpu && ((tombe(posx_arrondi, posy_arrondi) && !sauts(aPosX, aPosY, fMoveX, fMoveY)) || ((oKart.speed > 2) && oMap.horspistes.herbe && ralenti(posx_arrondi, posy_arrondi) && !oKart.protect && !oKart.champi))) {
				oKart.z = 1;
				oKart.heightinc = 0.5;
			}
		}
	...
	if ((page == "MK" && course=="BB" && oKart.cpu) || (oKart.z > 1.175) || canMoveTo(aPosX,aPosY, fMoveX,fMoveY, oKart.protect)) {
		...
		if (oKart.cpu)
			...
			for (var i=5;i>0;i--) {
				if (horizontality == "v")
					oKart.y += (oKart.y-fNewPosY)*i/oKart.speed;
				else
					oKart.x += (oKart.x-fNewPosX)*i/oKart.speed;
	...
	if (isChallengeOpen(5) && (oKart == oPlayers[0]) && !advent__cheated) {
		if ((course == "BB") && simplified && !isOnline && nid == 3472) {
			if (!oKart.z && oKart.x > 100 && oKart.y > 100 && oKart.x < 500 && oKart.y < 500) {
				challengeSucceeded(5,iScreenScale);
				advent__pauseIfNot();
			}
		}
	}
					...
					if (!oPlayers[1-getId] || oPlayers[1-getId].cpu) {
						if (!isOnline) {
							if (course != "CM") {
								...
								for (var i=0;i<nbjoueurs;i++) {
									for (var j=0;j<nbjoueurs;j++) {
										...
										if (aKarts[j].place == i+1) {
											...
											positions += '<tr id="fJ'+i+'" style="background-color: '+ (j<strPlayer.length ? (j ? (isRedTeam?'brown':'navy') : (isRedTeam?'#F96':'#69F')) : (isRedTeam?'red':'transparent')) +'"><td>'+ toPlace(i+1)+' </td><td class="maj" id="j'+i+'">'+ toPerso(joueur) +'</td><td id="pts'+i+'">'+ aScores[j] +'<small>+'+ ptsInc +'</small></td></tr>';
											aScores[j] += ptsInc;
											j = nbjoueurs;
										}
									}
								...
								if (isChallengeOpen(22) && !oPlayers[1] && (aKarts.length == 50) && !advent__cheated) {
									if ((course == "VS") && !isCup && !isOnline) {
										if (oPlayers[0].place == 37)
											challengeSucceeded(22,iScreenScale);
									}
								}
								if (isChallengeOpen(21) && !oPlayers[1] && (iDificulty == 5) && !iTeamPlay && (aKarts.length == 100) && !advent__cheated) {
									if ((course == "VS") && !isCup && !isOnline && oMap.map == 39) {
										if (oPlayers[0].place == 1)
											challengeSucceeded(21,iScreenScale);
									}
								}
								if (isChallengeOpen(20) && !oPlayers[1] && (iDificulty == 5) && !iTeamPlay && (aKarts.length == 8) && !advent__cheated) {
									if ((course == "VS") && !isCup && !isOnline && oMap.map == 1) {
										if (oPlayers[0].place == 1 && (advent__startedat > 298))
											challengeSucceeded(20,iScreenScale);
									}
								}
								if (isChallengeOpen(18) && !oPlayers[1] && (iDificulty == 5) && !iTeamPlay && (aKarts.length == 8) && !advent__cheated) {
									if ((course == "VS") && !isCup && !isOnline && oMap.map == 13) {
										if (oPlayers[0].place == 1 && !advent__itemed)
											challengeSucceeded(18,iScreenScale);
									}
								}
								if (isChallengeOpen(16) && !oPlayers[1] && (iDificulty == 5) && !iTeamPlay && (aKarts.length == 8) && !advent__cheated) {
									if ((course == "VS") && complete && !isOnline && oMap.map == 83) {
										if (oPlayers[0].place == 1 && oPlayers[0].initialPlace == 8)
											challengeSucceeded(16,iScreenScale);
									}
								}
								if (isChallengeOpen(14) && !oPlayers[1] && iTeamPlay && (aKarts.length == 8) && !advent__cheated) {
									if ((course == "VS") && !isCup && !isOnline) {
										var oneToFour = true;
										for (var i=0;i<aKarts.length;i++) {
											if (aKarts[i].team == oPlayers[0].team && aKarts[i].place > 4)
												oneToFour = false;
										}
										if (oneToFour)
											challengeSucceeded(14,iScreenScale);
									}
								}
								if (isChallengeOpen(13) && !oPlayers[1] && (iDificulty == 5) && !iTeamPlay && (aKarts.length == 8) && !advent__cheated) {
									if ((course == "VS") && !isCup && !isOnline && oMap.map == 1) {
										if (oPlayers[0].place == 1 && (advent__startedat > 223))
											challengeSucceeded(13,iScreenScale);
									}
								}
								if (isChallengeOpen(10) && !oPlayers[1] && (iDificulty == 5) && !iTeamPlay && (aKarts.length == 8) && !advent__cheated) {
									if ((course == "VS") && !isCup && !isOnline && oMap.map == 15) {
										if (oPlayers[0].place == 1 && (oPlayers[0].personnage == "luigi"))
											challengeSucceeded(10,iScreenScale);
									}
								}
								if (isChallengeOpen(7) && !oPlayers[1] && (iDificulty == 5) && !iTeamPlay && (aKarts.length == 8) && !advent__cheated) {
									if ((course == "VS") && !isCup && !isOnline && oMap.map == 29) {
										if (oPlayers[0].place == 1 && !advent__drifted)
											challengeSucceeded(7,iScreenScale);
									}
								}
								if (isChallengeOpen(6) && !oPlayers[1] && (iDificulty == 5) && !iTeamPlay && (aKarts.length == 8) && !advent__cheated) {
									if ((course == "VS") && complete && !isOnline && oMap.map == 73) {
										if (oPlayers[0].place == 1)
											challengeSucceeded(6,iScreenScale);
									}
								}
								if (isChallengeOpen(3) && !oPlayers[1] && (iDificulty == 5) && !iTeamPlay && (aKarts.length == 50) && !advent__cheated) {
									if ((course == "VS") && !isCup && !isOnline && oMap.map == 19) {
										if (oPlayers[0].place == 1)
											challengeSucceeded(3,iScreenScale);
									}
								}
							}
							else {
								if (isChallengeOpen(11) && !advent__cheated) {
									if ((course == "CM") && complete && !isOnline && oMap.map == 6921) {
										if (timer < 1120)
											challengeSucceeded(11,iScreenScale);
									}
								}
								if (isChallengeOpen(2) && !advent__cheated) {
									if ((course == "CM") && complete && !isOnline && oMap.map == 1) {
										if (timer < 896)
											challengeSucceeded(2,iScreenScale);
									}
								}
						...
						if (isChallengeOpen(15) && !advent__cheated) {
							if (simplified && !isOnline && (nid == 3673)) {
								if (advent__falls <= 5)
									challengeSucceeded(15,iScreenScale);
							}
						}
						if (isChallengeOpen(8) && !advent__cheated) {
							if (simplified && !isOnline && (nid == 3673)) {
								if (advent__falls <= 10)
									challengeSucceeded(8,iScreenScale);
							}
						}
						document.onkeydown = undefined;
						document.onkeyup = undefined;
		...
		if (!isOnline) {
			...
			if (gagnant) {
				...
				for (var i=0;i<aKarts.length;i++) {
					...
					positions += '<tr id="fJ'+i+'" style="background-color:'+ (i<strPlayer.length ? (i ? (isRedTeam?'brown':'navy') : (isRedTeam?'#F96':'#69F')) : (isRedTeam?'red':'transparent')) +'"><td>'+ toPlace(i+1) +' </td><td class="maj" id="j'+i+'">'+ toPerso(joueur) +'</td><td id="pts'+i+'">'+ aScores[i] + (!ptsInc ? "" : "<small>+1</small>")+'</td></tr>';
					aScores[i] += ptsInc;
				}
				...
				if (isChallengeOpen(24) && !oPlayers[1] && !iTeamPlay && (aKarts.length == 8) && !advent__cheated) {
					if ((course == "BB") && complete && !isOnline && nid == 588) {
						if (advent__kills >= 7)
							challengeSucceeded(24,iScreenScale);
					}
				}
				if (isChallengeOpen(17) && !oPlayers[1] && !iTeamPlay && (aKarts.length == 8)) {
					if ((course == "BB") && !isCup && !isOnline) {
						if (gagnant == oPlayers[0] && !advent__cheated) {
							advent__winnersince++;
							if (advent__winnersince == 4)
								challengeSucceeded(17,iScreenScale);
						}
						else
							advent__winnersince = 0;
					}
				}
				if (isChallengeOpen(12) && !oPlayers[1] && !iTeamPlay && (aKarts.length == 50) && !advent__cheated) {
					if ((course == "BB") && !isCup && !isOnline && oMap.map == 41) {
						if (gagnant == oPlayers[0])
							challengeSucceeded(12,iScreenScale);
					}
				}
				if (isChallengeOpen(9) && !oPlayers[1] && !iTeamPlay && (aKarts.length == 8) && !advent__cheated) {
					if ((course == "BB") && !isCup && !isOnline && oMap.map == 44) {
						if (gagnant == oPlayers[0] && (gagnant.ballons.length+gagnant.reserve >= 6))
							challengeSucceeded(9,iScreenScale);
					}
				}
				if (isChallengeOpen(1) && !oPlayers[1] && !iTeamPlay && (aKarts.length == 8) && !advent__cheated) {
					if ((course == "BB") && !isCup && !isOnline && oMap.map == 44) {
						if (gagnant == oPlayers[0] && (gagnant.ballons.length+gagnant.reserve >= 5))
							challengeSucceeded(1,iScreenScale);
					}
				}
			}
		}
...
var advent__cheated = false;
var advent__drifted = false;
var advent__itemed = false;
var advent__falls = 0;
var advent__hits = 0;
var advent__kills = 0;
var advent__killed = [];
var advent__startedat = 0;
var advent__myitems = [];
function advent__pauseIfNot() {
	setTimeout(function() {
		var stillRacing = (kartIsPlayer(oPlayers[0]) && !oPlayers[0].loose && !finishing) || willReplay;
		if (!pause && stillRacing) {
			document.getElementById("infos0").style.visibility = "visible";
			pause = true;
			if (bMusic || iSfx) {
				var oMusics = document.getElementsByClassName("gamemusic");
...
function openCheats() {
	var cheatCode = prompt("MKPC Console command"+(language ? "\nCareful, you're not allowed to use cheat codes for advent calendr challenges!":"\nAttention, les cheat codes sont interdits pour les défis du calendrier de l'avent !"));
	if (!cheatCode)
		return false;
	if (!processCode(cheatCode))
		alert("Invalid command");
	else advent__cheated = true;
}
...
var advent__fastforward = 0;
function cycle() {
	if (!pause) {
		if (advent__fastforward > timer)
			setTimeout(cycle,1);
		else
		setTimeout(cycle,67);
		for (var i=0;i<aKarts.length;i++) {
	...
	if (page == "MK") {
		...
		for (i=0;i<oModes.length;i++) {
			...
			oPInput.onclick = function() {
				...
				if (course == "CL")
					document.location.href = "online.php?"+(isSingle?(complete?"i":"id"):(complete?"cid":"sid"))+"="+nid;
				else {
					FBRoot.style.display = "none";
					oScr.innerHTML = "";
...
function searchCourse() {
	...
	for (var i=0;i<ratio;i++) {
		...
		oImg.style.imageRendering = "-moz-crisp-edges";
		oImg.style.imageRendering = "crisp-edges";
...
function selectRaceScreen(cup) {
	if (isOnline || (!isSingle && course != "GP")) {
		...
		if ((isCup || isBattle || (course == "BB")) && !isSingle) {
			var oPInput = document.createElement("input");
			oPInput.type = "button";
			if (course != "CM")
				oPInput.value = toLanguage("Random", "Aléatoire");
...
function selectOnlineTeams(strMap,choixJoueurs,selecter) {
	...
	if (selecter) {
		...
		function waitForSelection() {
			var teamsPayload = "";
			if (isBattle) teamsPayload = "battle";
			xhr("getTeams.php", teamsPayload, function(res) {
...
function setChat() {
	...
	bConnectes.onclick = function() {
		...
		xhr("listCoursePlayers.php", (course=="BB"?"battle":""), function(reponse) {
			if (reponse) {
				try {
					var rCode = eval(reponse);
				}
