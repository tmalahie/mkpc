...
function startGame() {
    ...
    var fakeNicks = ["Wargor", "Link-Triforce-8", "Zekirio", "Veethan23", "Conqueror", "InfiNate", "Toad64", "ByoT", "Neemo", "Fways", "Clad", "MudkiPro"];
	...
	for (var i=0;i<strPlayer.length;i++) {
		...
		oPlayer.nick = fakeNicks[i%fakeNicks.length];
		if (iTeamPlay)
	...
	for (i=0;i<aPlayers.length;i++) {
        ...
        oEnemy.nick = fakeNicks[inc%fakeNicks.length];
		if (iTeamPlay)
...
function classement() {
	...
	for (var i=0;i<iPlacement.length;i++) {
		...
		var tPlayer = aKarts[iPlayer].nick;
