...
if ($id) {
	...
	if ($getCourse = mysql_fetch_array(mysql_query('SELECT course,banned FROM `mkjoueurs` j LEFT JOIN mkmuted m ON j.id=m.player WHERE j.id="'.$id.'" AND m.player IS NULL'))) {
		if (!$getCourse['banned']) {
			...
			mysql_query('INSERT INTO `mkchatvoc` SET course='.$getCourse['course'].', player='.$id.', syncid=0, muted="'. $muted .'" ON DUPLICATE KEY UPDATE syncid=syncid+1, muted=VALUES(muted)');
