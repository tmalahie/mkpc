...
$limItemIdle = timeInFrames($timeMs-60);
$newItems = array();
if (!$finished) {
	...
	// uses apcu to run garbage collection only once per sec
	$gcKey = 'last_item_gc_time';
	$currentTime = time();

	$lastGcTime = apcu_fetch($gcKey); // fetch last GC time

	if ($lastGcTime !== $currentTime) {
		// already ran in this sec!
		apcu_store($gcKey, $currentTime);

		// delete items older than $limItemIdle sec
		mysql_query('DELETE FROM items WHERE course="' . $course . '" AND data="" AND updated_at<"' . $limItemIdle . '"');
	}
