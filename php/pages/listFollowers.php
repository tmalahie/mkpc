...
<main>
	...
	$getUsers = mysql_query('SELECT f.follower,DATE_FORMAT(f.date, "'. $dateFormat .'") AS infosDate FROM `mkfollowusers` f'. visibleFollowerJoin('f.follower') .' WHERE f.followed="'. $id .'" AND '. visibleFollowerCondition() .' ORDER BY f.date DESC');
	$users = array();
