    ...
    if (!empty($ignoredNotifs[$myNotif['type']])) {
        ...
        switch ($myNotif['type']) {
            ...
            case 'currently_online':
                ...
                break;
            case 'new_record':
                if ($rData = mysql_fetch_array(mysql_query('SELECT r.* FROM `mkrecords` r LEFT JOIN mkrecords r2 ON r2.identifiant=' . $myIdentifiants[0] . ' AND r2.identifiant2=' . $myIdentifiants[1] . ' AND r2.identifiant3=' . $myIdentifiants[2] . ' AND r2.identifiant4=' . $myIdentifiants[3] . ' AND r2.class=r.class AND r2.type=r.type AND r2.circuit=r.circuit AND r2.time<r.time WHERE r.id="' . $myNotif['link'] . '" AND r2.id IS NULL'))) {
...
require_once('../includes/circuits.php');
...
foreach ($notifsData as $i => $notifData) {
    if ($i >= $relyOnCacheThreshold)
        break;
    $notifData['link'] = preg_replace('#(?:&amp;|\?)src=\w+$#', '', $notifData['link']);
    $res[] = $notifData;
}
