...
<main>
	...
	<?php
	$records = mysql_query('SELECT t.player,j.nom,t.score,c.code FROM `mkttranking2` t INNER JOIN `mkjoueurs` j ON t.player=j.id INNER JOIN `mkprofiles` p ON p.id=j.id LEFT JOIN `mkcountries` c ON c.id=p.country WHERE '. ($joueur ? 'j.nom="'.$joueur.'"':'j.deleted=0') .' ORDER BY t.score DESC,t.player');
	if ($joueur) {
	...
	<?php
		if ($joueur) {
			$getPlaces = mysql_query('SELECT t.player FROM `mkttranking2` t INNER JOIN `mkjoueurs` j ON t.player=j.id WHERE (t.score>'.$record['score'].' OR (t.score='.$record['score'].' AND t.player<'.$record['player'].')) AND j.deleted=0');
