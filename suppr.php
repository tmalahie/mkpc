...
if (mysql_numrows(mysql_query('SELECT * FROM `circuits` WHERE id="'.$cID.'"'. (hasRight('moderator') ? '':' AND identifiant='.$identifiants[0].' AND identifiant2='.$identifiants[1].' AND identifiant3='.$identifiants[2].' AND identifiant4='.$identifiants[3])))) {
	...
	include('getExt.php');
	$id = $id2;
	unlink('images/uploads/map'.$cID.'.'.$ext);
