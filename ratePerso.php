...
if (isset($_POST['id']) && isset($_POST['rating'])) {
	...
	if ($rating >= 0 && $rating <= 5) {
		...
		mysql_query('UPDATE `mkchisto` SET rating="'. $rating .'" WHERE id="'.$persoId.'" AND identifiant="'. $identifiants[0] .'" AND identifiant2="'. $identifiants[1] .'" AND identifiant3="'. $identifiants[2] .'" AND identifiant4="'. $identifiants[3] .'"');
