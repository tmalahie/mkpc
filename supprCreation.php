...
if (isset($_POST['id'])) {
	...
	if (mysql_numrows(mysql_query('SELECT * FROM `mkcircuits` WHERE id="'. $cID .'"'. (hasRight('moderator') ? '':' AND identifiant='.$identifiants[0].' AND identifiant2='.$identifiants[1].' AND identifiant3='.$identifiants[2].' AND identifiant4='.$identifiants[3])))) {
		...
		if (hasRight('moderator'))
			mysql_query('INSERT INTO `mklogs` VALUES(NULL, '. $id .', "SCircuit '. $cID .'")');
	}
	echo 1;
	mysql_close();
}
